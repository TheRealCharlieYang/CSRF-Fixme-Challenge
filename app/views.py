...
# @csrf_exempt  # Temporarily uncommented to show original state
def create_post(request):
    ...
    # Handle case where user doesn't have a userdetail (create one if needed)
    try:
        creator = request.user.userdetail
    except UserDetail.DoesNotExist:
        creator = UserDetail.objects.create(user_id=request.user)
        creator.save()
